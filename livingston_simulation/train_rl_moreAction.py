...
from rl_env_moreAction import CoherentGoniometerEnv
...
def main(args):
    ...
    env = CoherentGoniometerEnv(
        ...
        action_penalty_ratio=args.action_penalty_ratio,
    )
    ...
    model = PPO(
        ...
        gamma=args.gamma,
        verbose=1,
        tensorboard_log="./moreAction_ppo_gonio_tensorboard/",
    )
    ...
    plot_callback = TrajectoryPlotCallback(
        eval_env=plot_env, 
        run_name=log_name,
        save_dir="./moreActionexperiments_plots",
        check_freq=20000)
    
    # eval callback
    eval_callback = EvalCallback(
        eval_env, 
        best_model_save_path=f'./moreAction_models/best_eval/{log_name}/',
        log_path='./moreAction_models/eval_logs/', 
        eval_freq=10_000, 
        n_eval_episodes=20,
        deterministic=True, 
        render=False)
    ...
    # Save
    model.save(f"./moreAction_models/{log_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--action_penalty_ratio", type=float, default=10, help="Coefficient for action penalty in the reward function")
